...
app = typer.Typer(help="garmin-glow-up — sync service")
...
def serve(
    ...
    print(f"GGU-READY token={token} port={port}", flush=True)
...
def sync(
    ...
    """Run one sync now."""
    from garmin_glow_up.sync.orchestrator import sync_all
...
def auth_login(
    ...
    from garmin_glow_up.auth.mfa import stdin_mfa
    try:
...
def auth_status():
    ...
    if tokens:
        ...
        typer.echo("Not authenticated. Run `garmin-glow-up auth login`.")
...
def backfill_training():
    ...
    from garmin_glow_up.auth.tokens import TokenStore
    from garmin_glow_up.db.models import Activity
    from garmin_glow_up.db.session import get_db
    from garmin_glow_up.garmin.client import GarminClient
    from garmin_glow_up.sync.activities import backfill_training_fields

    store = TokenStore()
    client = GarminClient(store)
    if not client.resume():
        typer.echo("Not authenticated. Run `garmin-glow-up auth login` first.", err=True)
        raise typer.Exit(1)
