...
import { useLoad, type LoadResponse } from "../hooks/useLoad";
...
const SPORT_LABELS: Record<string, string> = {
  run: "Run", bike: "Bike", swim: "Swim", trail: "Trail", lift: "Strength",
};
...
function getDelta(series: LoadResponse["series"], field: "ctl" | "atl" | "tsb", daysBack: number): string {
...
function BigStat({
  ...
  return (
    <div className="card" style={{ borderLeft: `2px solid ${tone}` }}>
      <h3 style={{ color: tone, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", margin: "0 0 6px" }}>{label}</h3>
      <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
        <span style={{ fontSize: 38, fontWeight: 550, letterSpacing: "-0.03em", lineHeight: 1, color: "var(--fg-0)" }}>{value}</span>
        {delta && (
...
function RampCard({ rampPerWeek }: { rampPerWeek: number }) {
  ...
  return (
    <div className="card" style={{ display: "flex", flexDirection: "column" }}>
      <h3 style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", margin: "0 0 6px", color: "var(--fg-2)" }}>
        Ramp Rate
      </h3>
      <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
        <span style={{ fontSize: 28, fontWeight: 550, letterSpacing: "-0.02em", color: "var(--fg-0)" }}>
          {sign}{rampPerWeek.toFixed(1)}
        </span>
      ...
      <div style={{ marginTop: 10, height: 8, borderRadius: 999, background: "linear-gradient(90deg, var(--z2) 0%, var(--z2) 50%, var(--z4) 75%, var(--z5) 100%)", position: "relative" }}>
        <div style={{ position: "absolute", left: `${pct}%`, top: -3, width: 2, height: 14, background: "var(--fg-0)", borderRadius: 1 }} />
      </div>
  ...
  return (
    <div className="card" style={{ display: "flex", flexDirection: "column", padding: "14px 18px" }}>
      <div style={{ display: "flex", alignItems: "center", marginBottom: 10 }}>
        <h3 style={{ margin: 0, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--fg-2)" }}>
          Performance Management
        </h3>
        <div style={{ marginLeft: "auto", display: "flex", gap: 12, fontSize: 11, color: "var(--fg-2)", alignItems: "center" }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
            <span style={{ width: 12, height: 2, background: "var(--run)", display: "inline-block" }} />CTL
          </span>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
            <span style={{ width: 12, height: 0, borderTop: "2px dashed var(--bike)", display: "inline-block" }} />ATL
          </span>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
            <span style={{ width: 12, height: 2, background: "var(--swim)", display: "inline-block" }} />TSB
          </span>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
            <span style={{ width: 7, height: 7, background: "var(--fg-3)", borderRadius: 1, display: "inline-block" }} />daily TSS
          </span>
        </div>
      </div>
      <div style={{ position: "relative", minHeight: 240 }}>
        ...
        >
          ...
          {[40, 80, 120, 160].map((y) => (
            <line key={y} x1="0" y1={y} x2="1140" y2={y} stroke="var(--line-soft)" strokeDasharray="2 4" />
          ))}
          ...
          {phaseLines.map((x) => (
            <line key={x} x1={x} y1="20" x2={x} y2="270" stroke="var(--line)" strokeDasharray="1 4" />
          ))}
          <g fontFamily="var(--font-mono)" fontSize="10" fill="var(--fg-2)">
            {phases.map(({ x, name }) => (
              <text key={name} x={x} y="14">{name}</text>
            ))}
          </g>
          ...
          {/* Now marker */}
          <line x1="1140" y1="0" x2="1140" y2="280" stroke="var(--fg-0)" strokeOpacity="0.35" strokeDasharray="2 3" />
          <circle cx="1140" cy={nowY} r="4" fill="var(--bg-0)" stroke="var(--run)" strokeWidth="2" />

          {/* X-axis date labels */}
          <g fontFamily="var(--font-mono)" fontSize="9" fill="var(--fg-3)">
            {axisLabels.map(({ x, label, anchor }) => (
              <text key={label} x={x} y="276" textAnchor={anchor as "start" | "middle" | "end"}>{label}</text>
            ))}
          </g>
        </svg>
      </div>
...
function WeeklyVolume({ weekly }: { weekly: LoadResponse["weekly"] }) {
  ...
  const maxTotal = Math.max(...shown.map((w) => SPORTS.reduce((s, sp) => s + (w.bySport[sp] ?? 0), 0)), 1);
    ...
    <div className="card">
      <div style={{ display: "flex", alignItems: "center", marginBottom: 10 }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
          {SPORTS.map((sp) => (
            <span key={sp} style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, color: "var(--fg-2)" }}>
              <span style={{ width: 8, height: 8, borderRadius: 2, background: `var(--${sp})`, display: "inline-block" }} />
              {SPORT_LABELS[sp]}
            </span>
          ))}
        </div>
      </div>
      <svg width="100%" height="80" viewBox={`0 0 1140 80`} preserveAspectRatio="none" aria-hidden="true">
        {shown.map((w, i) => {
          ...
          return (
            <g key={w.weekStart}>
              {[...SPORTS].map((sp) => {
                ...
                y -= h;
                return <rect key={sp} x={x} y={y} width={barW} height={h} fill={`var(--${sp})`} opacity="0.9" />;
              })}
              <text x={x + barW / 2} y={78} textAnchor="middle" fontSize="9" fontFamily="var(--font-mono)" fill="var(--fg-3)">
                w{i + 1}
              </text>
            </g>
          );
...
export function TrainingLoad() {
  ...
  const rampPerWeek = data && data.series.length >= 8
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      <TopBar
        crumbs={["Library", "Training load", RANGE_LABELS[range]]}
        right={rangeSelector}
      />
        ...
        {data && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
              ...
              <BigStat
                label="TSB · Form"
                value={data.current.tsb > 0 ? `+${data.current.tsb.toFixed(0)}` : data.current.tsb.toFixed(0)}
                delta={tsbDelta}
                tone="var(--swim)"
                desc="CTL − ATL"
              />
